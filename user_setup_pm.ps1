...
# Use the Common Programs folder so DesktopApplicationLinkPath
# can consistently reference %ALLUSERSPROFILE%.
...
$UserTaskbarLayoutPath = Join-Path `
...
try {
    foreach ($Directory in @(
        $TaskbarShortcutDirectory
        $TaskbarLayoutDirectory
        $UserShellDirectory
    )) {
    ...
    Write-Success "Taskbar directories were initialized."
}
catch {
    Write-Failure (
...
# Remove shortcuts generated by a previous execution.
# This prevents old or renamed shortcuts from remaining in the folder.
try {
    Get-ChildItem `
        -LiteralPath $TaskbarShortcutDirectory `
        -Filter "*.lnk" `
        -File `
        -ErrorAction SilentlyContinue |
        Remove-Item `
    ...
    Write-Warning (
        "Unable to remove old taskbar shortcuts: " +
        $_.Exception.Message
    )
}

# Taskbar layout must be based on all configured taskbar applications,
# not only applications selected for installation on this execution.
#
# Otherwise, when -ApplicationNames is used together with
# PinListPlacement="Replace", unselected taskbar applications would
# disappear from the resulting layout.
$TaskbarApplicationDefinitions = @(
    $ApplicationDefinitions |
        Where-Object {
            $_.CreateTaskbarShortcut -eq $true
        }
)

foreach ($Application in $TaskbarApplicationDefinitions) {
    $ApplicationInstalled = Test-ApplicationInstalled `
        -DisplayNamePatterns $Application.DisplayNamePatterns `
        -ExecutablePaths $Application.ExecutablePaths `
        -AppxNamePatterns $Application.AppxNamePatterns `
    ...
    # Microsoft Store and packaged Windows applications use AUMID.
    if (
        -not [string]::IsNullOrWhiteSpace(
            $AppUserModelId
        )
    ) {
        $EscapedAppUserModelId = (
...
if ($TaskbarPinEntries.Count -eq 0) {
    Write-Skip (
        "No installed applications are configured for taskbar pinning."
    )
}
else {
...
"@
    ...
    try {
        ...
        # explicitly write UTF-8 with BOM.
        ...
        Copy-Item `
            ...
            -Destination $UserTaskbarLayoutPath `
            -Force `
            -ErrorAction Stop
        ...
        Write-Success (
            "Taskbar layout was copied to: " +
            $UserTaskbarLayoutPath
        )

        # Apply the layout to the currently logged-in user.
        # HKCU is required here because the script is configuring the
        # current user's taskbar and Explorer session.
        $ExplorerPolicyPath = (
        ...
        Write-Success (
            "The current-user taskbar layout policy was configured."
        )
        ...
        try {
            ...
            Write-Success "The current-user policy was refreshed."
        }
        catch {
            Write-Warning (
                "Unable to refresh the current-user policy: " +
                $_.Exception.Message
            )
        }

        # Restart Explorer so the current user taskbar reloads the
        # generated layout.
        Get-Process `
            -Name "explorer" `
            -ErrorAction SilentlyContinue |
        ...
        Write-Success (
            "Windows Explorer was restarted to reload the taskbar."
        )

        Write-Warning (
            "If the taskbar order is not updated immediately, " +
